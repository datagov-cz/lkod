...
define('LOGFILE', '/data/webhook-logs/lkod-from-github.log');

// what command to execute upon retrieval of a valid push event
$cmd = 'cd <GITHUB_REPO_DIRECTORY> && git fetch --all && git checkout --force "origin/main" && curl -X POST http://localhost:8080/resources/executions?pipeline=<IRI_PIPELINE>';

// the shared secret, used to sign the POST data (using HMAC with SHA1)
$secret = '<SECRET_GITHUB_WEBHOOK>';
...
$required_data = array(
        'ref' => 'refs/heads/main',
        'repository' => array(
                'full_name' => '<GitHubOrg/GitHubRepo>',
        ),
);
